...
<template>
  <div id="vote-register">
    <van-nav-bar
      :title="$t('vote.register.navTitle')"
      left-arrow
      fixed 
...
export default {
  ...
  methods:{
    ...
    nameChange() {
      if (this.name == '') {
        this.msg = this.$t('commom.tx.msg1');
        this.status = false;
      } else if (new Buffer(this.name).length > 32) {
        this.msg = this.$t('commom.tx.msg2');
        this.status = false;
      } else {
        this.msg = '';
        this.status = true;
    ...
    makeTX(currentAddr) {
      ...
      let OP_RETURN = this.LBTCtools.RegForgedNode(this.name);
    ...
    makeOpReturn(param, type) {
      ...
      header    = '00000000';
      code      = 'c' + type;
      name      = this.stringToHex(param);
      name_len  = Number(this.bign(name.length).div(2).toString()).toString(16);
      if (String(name_len).length == 1) {
        name_len = '0' + name_len
      }
      data = code + name_len + name;
      let OP_RETURN = header + data;
      return OP_RETURN
    },
